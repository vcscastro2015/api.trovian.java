...
--changeset trovian:v002-inserir-templates-sistema
--comment: Inserção dos templates de sistema padrão

-- Template 1: Despesas por Veículo (Financeiro)
INSERT INTO relatorio_templates (nome, descricao, categoria, query_base, query_campos, query_joins, query_order_by, ativo, sistema_template, created_at)
VALUES (
...
--rollback DELETE FROM campos_relatorio WHERE template_id IN (1, 2, 3, 4);
--rollback DELETE FROM parametros_relatorio WHERE template_id IN (1, 2, 3, 4);
--rollback DELETE FROM relatorio_templates WHERE sistema_template = TRUE;
