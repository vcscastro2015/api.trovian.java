...
public class DocumentoVeiculoListener {

    private final ObjectMapper objectMapper;
    private final DocumentoVeiculoService documentoVeiculoService;

    @JmsListener(destination = JmsConfig.DOCUMENTOS_VEICULO)
    public void receiveDocumentoVeiculo(Message message) {
        try {
            String mensagemJson;
            if (message instanceof TextMessage) {
                ...
                throw new IllegalStateException("Tipo JMS não suportado: " + message.getClass());
            }
            DadosFilaDTO dadosFilaDTO = objectMapper.readValue(mensagemJson, DadosFilaDTO.class);
            documentoVeiculoService.processarDocumentoWhatsApp(dadosFilaDTO);
        } catch (Exception e) {
            throw new RuntimeException("Erro ao processar DocumentoVeiculo", e);
        }
    }
}
